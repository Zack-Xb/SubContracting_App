 ...
 const handleImgUpload=async()=>{
   ...
   if(!result.cancelled){
    ...
    const handleProfileUpload=async()=>{
     ...
     const handlePackage=()=>{
      ...
      setDoc(doc(db,'Users',auth.currentUser.uid,'Packages',docPackage),{
      ...
      setDoc(doc(db,'Packages',docPackage),{
        displayImage: docPackage,
        profileImage: auth.currentUser.uid,
        name: auth.currentUser.displayName,
        title: title,
  ...
  if(isTabletOrMobileDevice){
      ...
      return(
        ...
        <Layout minH={true}>
          ...
          <Appbar.BackAction onPress={() => navigation.navigate('SB') } color='#fff' />
           <Appbar.Content title={'List Package'} titleStyle={style.title}/>
          </Appbar>
    ...
    />
           ...
           <TextInput  label={'Input Sol Rate'} style={style.titleInput} value={rate} onChangeText={text => setRate(text)}/>
